...
static sgx_status_t SGX_CDECL sgx_ecall_function(void* pms)
{
	...
	if (_tmp_str != NULL && _len_str != 0) {
		...
		if ((_in_str = (char*)malloc(_len_str)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_str, 0, _len_str);
	}

	ecall_function(_in_str, _tmp_len);
	if (_in_str) {
		if (memcpy_s(_tmp_str, _len_str, _in_str, _len_str)) {
...
sgx_status_t SGX_CDECL ocall_function(char* buf)
{
	sgx_status_t status = SGX_SUCCESS;
	size_t _len_buf = buf ? strlen(buf) + 1 : 0;

	ms_ocall_function_t* ms = NULL;
	size_t ocalloc_size = sizeof(ms_ocall_function_t);
	void *__tmp = NULL;
	...
	ms = (ms_ocall_function_t*)__tmp;
	__tmp = (void *)((size_t)__tmp + sizeof(ms_ocall_function_t));
	ocalloc_size -= sizeof(ms_ocall_function_t);
